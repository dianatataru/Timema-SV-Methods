...
### Permutation test on longest run ###
permutation_test <- function(outlier_df, mds_df, n_perm = 1000, p_thresh = 0.01,
                             max_gap = 2, min_windows = 5) {
  longest_run <- function(sorted_idx, max_gap) {
    if (length(sorted_idx) < 1L) return(0L)
    gaps <- diff(sorted_idx) - 1L
    max(tabulate(cumsum(c(1L, gaps > max_gap))))
  }

  all_idx <- seq_len(nrow(mds_df))
  results <- list()

  for (ax in unique(outlier_df$mds_axis)) {
    for (dir in c("pos", "neg")) {
      sub <- outlier_df[outlier_df$mds_axis == ax & outlier_df$direction == dir, ]
      if (nrow(sub) < min_windows) next

      K        <- nrow(sub)
      flag_idx <- sort(match(sub$win_index, mds_df$win_index))

      L_obs  <- longest_run(flag_idx, max_gap)
      L_perm <- replicate(n_perm, longest_run(sort(sample(all_idx, K)), max_gap))
      p_val  <- mean(L_perm >= L_obs)

      if (p_val >= p_thresh) next

      cat("Significant clustering:", ax, dir, "| p =", p_val, "\n")

      gaps   <- diff(flag_idx) - 1L
      run_id <- cumsum(c(1L, gaps > max_gap))

      for (rid in unique(run_id)) {
        sel <- run_id == rid
        if (sum(sel) < min_windows) next

        idx      <- flag_idx[sel]
        win_rows <- sub[match(mds_df$win_index[idx], sub$win_index), ]
        win_rows <- win_rows[!is.na(win_rows$win_index), ]
        if (nrow(win_rows) == 0) next

        results[[length(results) + 1]] <- data.frame(
          mds_axis  = ax,
          direction = dir,
          chrom     = win_rows$chrom[1],
          start_pos = min(win_rows$start_pos),
          end_pos   = max(win_rows$end_pos),
          n_windows = nrow(win_rows),
...
get_outlier_runs <- function(outlier_df, min_windows = 5, max_gap= 2) {
  ...
  for (chr in unique(outlier_df$chrom)) {
    ...
    gaps   <- c(0, diff(sub$win_index)-1)
    run_id <- cumsum(gaps > max_gap)
...
run_region_pca <- function(run_df, coded_matrix, win_size) {
  ...
  mat      <- coded_matrix[all_snps, , drop = FALSE]
...
make_pca_plots <- function(outlier_runs, coded_matrix, win_size, mds_axis) {
  ...
  for (rkey in unique(outlier_runs$region_key)) {
    ...
    n_wins       <- nrow(run_df)
    region_chrom <- run_df$chrom[1]
    region_start <- run_df$start_pos[1]
    region_end   <- run_df$end_pos[nrow(run_df)]
...
for (ax in 1:n_axes) {
  ...
  ggsave(paste0(output_prefix, "_", win_size, "snp_", mds_col, "_plot.pdf"), p)
  ...
  all_outlier_rows[[ax]] <- data.frame(
    mds_axis  = mds_col,
    direction = ifelse(outlier_wins[[z_col]] > 0, "pos", "neg"),
...
### Write summary table and run filtering ###
if (length(all_outlier_rows) > 0) {
  ...
  ### Run filtering ###
  cat("\nRunning permutation filtering...\n")
  final_inversions <- permutation_test(outlier_table, mds_df,
                                   n_perm      = 1000,
                                   p_thresh    = 0.01,
                                   max_gap     = 2,
                                   min_windows = 5)

  write.table(final_inversions,
                file      = paste0(output_prefix, "_", win_size, "snp_putative_inversions.txt"),
                sep       = "\t",
                row.names = FALSE,
                quote     = FALSE)

  cat("\nPutative inversions after permutation test:\n")
  print(final_inversions)
}

cat("\nAfter filtering, axes with outliers:   ", paste(paste0("MDS", axes_with_outliers), collapse = ", "), "\n")
cat("After filtering, axes without outliers:", paste(paste0("MDS", setdiff(1:n_axes, axes_with_outliers)), collapse = ", "), "\n")

### make pca of final putative inversions ###
cat("\nGenerating PCAs for putative inversions...\n")
for (i in 1:nrow(final_inversions)) {
  inv <- final_inversions[i, ]
    
  # Get all outlier windows in this region from outlier_table
  region_wins <- outlier_table[
    outlier_table$mds_axis == inv$mds_axis &
    outlier_table$chrom    == inv$chrom &
    outlier_table$start_pos >= inv$start_pos &
    outlier_table$end_pos   <= inv$end_pos, ]
    
  if (nrow(region_wins) < 5) next
    
  # Add required columns for make_pca_plots
  region_wins$run_id     <- 1
  region_wins$region_key <- paste(region_wins$chrom, 1, sep = "__")
  z_col <- paste0("z_", inv$mds_axis)
  if (!z_col %in% colnames(region_wins)) {
    region_wins[[z_col]] <- region_wins$z_score
  }
    
  pca_plots <- make_pca_plots(region_wins, coded, win_size, inv$mds_axis)
    
  if (length(pca_plots) > 0) {
  pdf(paste0(output_prefix, "_", win_size, "snp_inversion_pca_",
               inv$mds_axis, "_", inv$chrom, ".pdf"),
        width = 7, height = 6)
