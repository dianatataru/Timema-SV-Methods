...
#make positions file 
node_ids <- as.integer(readLines("positions.txt"))
...
outlier_table <- data.frame(
  ...
  node_start = positions$pos[(outlier_wins - 1) * 100 + 1],
  node_end   = positions$pos[pmin(outlier_wins * 100, nrow(coded))],
...
for (w in outlier_wins) {
  ...
  plot(pca_out$x[, 1], pca_out$x[, 2],
       ...
       pch = 19,
       xlab = "PC1", ylab = "PC2",
       main = paste("Window", w, "| nodes", 
                    positions$pos[snp_start], "-", positions$pos[snp_end]))
